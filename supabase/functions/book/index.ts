...
interface BookBody {
  ...
  timeSlot?: string; // 24h "HH:MM", e.g. '09:00'
...
Deno.serve(async (req) => {
  ...
  // Reject a slot someone else already holds (declined/refunded rows free the slot).
  if (body.timeSlot) {
    const { data: clash } = await admin
      ...
      .limit(1);
    if (clash && clash.length > 0) return Response.json({ error: 'slot_taken' }, { status: 409 });
  }
